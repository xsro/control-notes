...
\begin{frame}{自适应控制器的定义与理论核心}
	\begin{itemize}
		...
		\item 【翻译】从这个意义上说，自适应控制器是应对\textbf{参数不确定性}的一种方法（有些方法需要将不确定性估计出来\footnote{【CAA云讲座预告】北京航空航天大学教授王薇：数据驱动自适应控制理论及应用}）
...
\begin{frame}
	\frametitle{自适应相关名词}
	Self-tuning 自整定、Auto-tuning 自动整定
	...
	\begin{columns}[t]
		...
		\begin {itemize}
		\item 自适应控制和增益调度控制都用于时变动态系统的控制
		\item 若动态特性随运行工况已知、可预测的方式变化，采用 \textbf {增益调度}\item 若使用固定控制器无法在 \textbf {鲁棒性} 和 \textbf {性能} 之间达成满意的折中，当且仅当此时，才应使用自适应控制。
		\end{itemize}
	\end{columns}
\end{frame}
